# Progress archive — M0 & M1 (detailed log)

Moved verbatim from `docs/PROGRESS.md` on 2026-08-16 to keep the live log lean. Covers
milestones M0 (`d794a3e`) and M1 (`78b0ffd`) plus the post-M1 debt sweep: the per-step
entries with full verification evidence, the milestone summaries, and the closed owner
decisions. The live `docs/PROGRESS.md` keeps only condensed summaries; this file is the
audit trail and is never edited, only appended to by future archival moves.
