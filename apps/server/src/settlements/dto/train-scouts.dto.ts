...
// live here. `unitType` stays a bare string rather than `@IsIn(UNIT_TYPES)` — an unknown or
// wrong-faction unit type must still reach `SettlementsService.trainScouts` and come back as
// `{ error: { key: 'errors.training.unknownType' | 'errors.training.wrongFaction', ... } }`
