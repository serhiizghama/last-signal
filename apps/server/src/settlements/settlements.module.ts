...
export class SettlementsModule implements OnModuleInit {
  constructor(
    ...
    @Inject(StarvationTickHandler) private readonly starvationTickHandler: StarvationTickHandler,
  ) {}

  // Registers the `buildComplete`, `trainingComplete` and `starvationTick` handlers with the
  // shared scheduler registry — the pattern the registry's own doc comment describes: feature
  // modules plug in from their own `onModuleInit`.
  onModuleInit(): void {
    this.registry.register(this.buildCompleteHandler);
    this.registry.register(this.trainingCompleteHandler);
    this.registry.register(this.starvationTickHandler);
  }
}
